...
class FTDController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'amount' => "nullable|numeric|min:50|max:999999999",
    ...
    public function check_leads_store(Request $request) {
        ...
        foreach($importData_arr as $key => $client) {
            ...
            $clients[$key][3] = $client[3];
            $clients[$key][4] = isset($client[4]) ? $client[4] : "";
            $is_leads_email = Lead::where('email', $client[2])->first();
            if(isset($is_leads_email) && $client[2] != "" && isset($client[2])){
                $clients[$key][5] = 'YES';
                $clients[$key][6] = date('Y-m-d', strtotime($is_leads_email->created_at));
                ...
                $clients[$key][7] = implode(' ', $source);
            }
            else {
                $is_leads_phone = Lead::where('phone', 'like', '%'.$client[3].'%')->first();
                if(isset($is_leads_phone) && $client[3] != "0" && isset($client[3])){
                    $clients[$key][5] = 'YES';
                    $clients[$key][6] = date('Y-m-d', strtotime($is_leads_phone->created_at));
            ...
            if($clients[$key][5] == 'YES' || ($clients[$key][4] != "" && str_contains(strtolower($clients[$key][4]),'crm'))) {
                if(isset($responses[(int) preg_replace('/[^0-9 ]/m','',$client[0])])){
                    if($responses[(int) preg_replace('/[^0-9 ]/m','',$client[0])][0]['amount']>0){
                        $clients[$key][8] = date('Y-m-d', strtotime($responses[(int) preg_replace('/[^0-9 ]/m','',$client[0])][0]['time']));
                        $clients[$key][9] = (int) $responses[(int) preg_replace('/[^0-9 ]/m','',$client[0])][0]['amount'];
                        ...
                        foreach($responses[(int) preg_replace('/[^0-9 ]/m','',$client[0])] as $client_transaction){
                            if($client_transaction['amount'] > 0){
                                ...
                                if(date_create($client_transaction['time']) >= date_add(date_create($clients[$key][8]),date_interval_create_from_date_string("7 days"))){
                                    $clients[$key][13] += $client_transaction['amount'];
                                }
                            } else {
                                $clients[$key][13] += $client_transaction['amount'];
                                if(date_create($client_transaction['time']) >= date_add(date_create($clients[$key][8]),date_interval_create_from_date_string("7 days"))){
                                    $clients[$key][15] += $client_transaction['amount'];
                                }
                            }
                        }
                        $clients[$key][12] = $clients[$key][9] + $clients[$key][10] + $clients[$key][11];
                        $clients[$key][15] = $clients[$key][13] + $clients[$key][14];
                        $clients[$key][11] = abs($clients[$key][11]);
                        $clients[$key][14] = abs($clients[$key][14]);
                    }
                }
            }
        }
